    ...
    return {
        ...
        controller: function ($scope,$location, $mdDialog, Auth) {
            ...
            $scope.getTopics = function () {
                $http.get('https://api.bschat.guillaumeperes.fr/api/user/'+$scope.user.id+'/topic').then(function(response) {
            ...
            $scope.getContacts = function () {
                $http.get('https://api.bschat.guillaumeperes.fr/api/user/'+$scope.user.id+'/contacts/get').then(function(response) {
                    $scope.contacts = response.data;
                })
            };

            $scope.getRequests = function () {
                $http.get('https://api.bschat.guillaumeperes.fr/api/user/'+$scope.user.id+'/requests').then(function(response) {
            ...
            $scope.acceptRequest = function (idRequest) {
                $http.get('https://api.bschat.guillaumeperes.fr/api/user/'+$scope.user.id+'/request/'+idRequest+'/accept').then(function(response) {
            ...
            $scope.refuseRequest = function (idRequest) {
                $http.get('https://api.bschat.guillaumeperes.fr/api/user/'+$scope.user.id+'/request/'+idRequest+'/refuse').then(function(response) {
            ...
            $scope.openAddTopicDialog = function (ev) {
                ...
                function addTopicModalController($scope, $mdDialog, $rootScope, Auth) {
                    ...
                    $scope.createTopic = function () {
                        ...
                        $http.post('https://api.bschat.guillaumeperes.fr/api/user/'+ $scope.user.id+'/topic', formData)
                            .then(function(response) {
                                var topic = response.data;
                                $mdDialog.hide();
                                window.location = 'https://bschat.guillaumeperes.fr/#/topic/'+topic.id;
                        })
